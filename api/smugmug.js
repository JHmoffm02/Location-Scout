...
#lib-grid{position:absolute;top:50px;left:0;right:0;bottom:0;overflow-y:auto;padding:20px;display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:16px;align-content:start}
#lib-grid::-webkit-scrollbar{width:3px}
#lib-grid::-webkit-scrollbar-thumb{background:var(--border2);border-radius:2px}

/* ── Cards ── */
.gcard{background:var(--bg3);border:1px solid var(--border);border-radius:var(--radius);overflow:hidden;cursor:pointer;transition:border-color .15s;min-width:0}
...
<div id="shell">
  ...
  <button id="sync-square" onclick="startFullSync()" title="Sync library" style="display:none;position:fixed;top:0;right:0;width:44px;height:92px;background:var(--bg2);border:none;border-left:1px solid var(--border);border-bottom:1px solid var(--border);cursor:pointer;z-index:50;color:var(--accent);font-size:10px;font-family:'DM Mono',monospace;letter-spacing:.05em;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px;transition:background .15s" onmouseover="this.style.background='var(--bg3)'" onmouseout="this.style.background='var(--bg2)'">
...
function updateAuthUI(ok){
  ...
  if(ok){b.textContent='smugmug ✓';b.classList.add('connected');if(sq)sq.style.display='flex';}
